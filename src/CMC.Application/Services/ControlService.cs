	...
	public class ControlService
	{
        ...
        public async Task<ControlDto> CreateAsync(CreateControlRequest r, CancellationToken ct = default)
        {
            var control = new Control(
                ...
                // ΔEAL/Score werden serverseitig später berechnet
            );

            // optional: Initialstatus über Transition (validiert Regeln)
            if (!string.IsNullOrWhiteSpace(r.InitialStatusTag))
                control.TransitionTo(r.InitialStatusTag!, DateTimeOffset.UtcNow);

            // optionales Fälligkeitsdatum
            if (r.DueDate.HasValue) control.SetDueDate(r.DueDate);
        ...
        public async Task<ControlDto?> UpdateAsync(UpdateControlRequest r, CancellationToken ct = default)
        {
            ...
            // Status NUR via Transition setzen (Request: StatusTag)
        ...
        public async Task<ControlDto?> TransitionAsync(ChangeControlStatusRequest r, CancellationToken ct = default)
        {
            ...
            var asOf = r.AsOfUtc ?? DateTimeOffset.UtcNow; // Fallback einbauen
            ...
            CancellationToken ct = default)
        {
            ...
            foreach (var lib in libs)
            {
                ...
                // konsistent per Transition auf „proposed“
